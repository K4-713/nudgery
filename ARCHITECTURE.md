...
**Setup-only sharing (`.nudge` files):** `ExportAnswersUseCase.executeSetupOnly()` emits the same JSON backup format but with an empty `answers` array, so no user data is shared. The detail screen's top app bar offers a dedicated Share icon ("Share nudge setup"), which writes the setup-only JSON to a cache file with a `.nudge` extension and launches an `ACTION_SEND` share sheet. On the receiving side, `AndroidManifest.xml` registers `ACTION_VIEW` intent filters for `content://` URIs with `application/octet-stream` and `application/json` MIME types; `MainActivity.handleNudgeFileIntent()` checks the `.nudge` extension at runtime (since `pathPattern` doesn't work with content URIs) and routes the content to the existing import flow via `SettingsViewModel.importNudgeFromBackup()`.
