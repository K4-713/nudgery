...
| `CalendarHeatMap` | `CalendarHeatMapChart` | Custom Canvas grid; layout varies by timeframe. Scrolling timeframes use fixed-row layouts per `HeatMapGranularity`: `SINGLE_DAY` 2-row day strip (weekly), `DAY` Monday-anchored 7-row grid (monthly), `WEEK_GRID` `WEEK_GRID_ROWS`-row week grid showing ~a year (`WEEK_GRID_VISIBLE_COLS` columns) per screen (yearly). All-time (`fillViewport`) instead lays the adaptive unit's cells (`SINGLE_DAY`/`DAY` → days, `WEEK` → weeks, `MONTH` → months) into an auto-fit grid (`fitHeatGrid`) that maximizes square cell size to fill the canvas with no scrolling. Cell color interpolated through the active `ChartPalette` by intensity (value ÷ max). When values exceed 0/1 a gradient scale bar labels the color→value mapping; tapping a cell outlines it and shows its exact date and value |
