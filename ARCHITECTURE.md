...
| SCALE | Line graph, calendar heat map (daily average; heat colors anchored to the question's defined min/max, week/month buckets average — `CalendarHeatMap.colorScaleMin/Max` + `HeatMapBucketAggregation.AVERAGE`, per DESIGN.md "Heat Map Value-to-Color Scaling") |
| NUMBER | Line graph, calendar heat map (daily average; heat colors fit the observed cells, week/month buckets sum) |
...
| `PackedBubble` | `PackedBubbleChart` | Custom Canvas with d3-style front-chain circle packing (`packSiblings` in `ui/screen/BubblePacking.kt`); radius ∝ `sqrt(NamedCount.count)` (area encodes frequency); cluster scaled to fit; bold centered word with count beneath. The packing places every bubble tangentially with **no overlaps** — the chart's basic premise, so no bubble is ever hidden behind another (covered by `BubblePackingTest`). Window scrubs **morph** between successive packings (`ui/screen/BubbleTransition.kt`, covered by `BubbleTransitionTest`): bubbles are matched by label and glide/resize, arrivals inflate in place, departures deflate beneath the survivors, and a shift arriving mid-morph retargets from the in-flight geometry (DESIGN.md "Packed bubble scrub transitions") |
| `CalendarHeatMap` | `CalendarHeatMapChart` | Custom Canvas grid. The **weekly** view is the only non-`fillViewport` layout: it draws its seven `SINGLE_DAY` cells as a single centered row spanning the card width. Every other timeframe sets `fillViewport` and lays the granularity's unit cells (`SINGLE_DAY`/`DAY` → days, `WEEK`/`WEEK_GRID` → weeks, `MONTH` → months) into an auto-fit grid (`fitHeatGrid`) that maximizes square cell size to fill the canvas with no scrolling. Week-bucketed cells (`WEEK`/`WEEK_GRID`, via `buildWeekCells`) count 7-day periods from the data-collection start (earliest answer, `CalendarHeatMap.weekAnchor`), not calendar Mondays, so the first cell is a full week from when logging began rather than a partial week clipped by a mid-week window edge. The yearly view spaces its month labels to show ~`WEEK_GRID_TARGET_MONTH_LABELS` markers across the width. Week/month buckets aggregate per `CalendarHeatMap.bucketAggregation` (sum for tallies, average for scale questions). Cell color interpolated through the active `ChartPalette` at the fraction computed by `HeatColorScale` (`ui/screen/HeatColorScale.kt`): fixed to the question's defined range for scale questions, fitted to the observed cells otherwise; zero renders as the "recorded zero" grey only when zero is the bottom of the range (DESIGN.md "Heat Map Value-to-Color Scaling"). A gradient scale bar labeled with the range's min/max appears when it carries information (always for fixed ranges with data; for observed fits, once values exceed 0/1); tapping a cell outlines it and shows its exact date and value |
