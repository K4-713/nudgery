...
- [ ] Data: build the `emoji-test.txt` → shared categorized-list generator + the runtime `hasGlyph` capability filter (spike this first to confirm maintenance feels light before building the picker UI). Have the generator report the gendered-concept count as a sanity check (spike measured ~74 man/woman-paired concepts in v16.0).
